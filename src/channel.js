...
const cardUpdateTimestamps = new Map();
...
const cardUpdateTimeouts = new Map();
...
let cleanupIntervalId = setInterval(cleanupCardCache, 30 * 60 * 1000);
...
async function sendProactiveMessage(config, target, text, optionsOrLog = {}) {
...
// Download media file
async function downloadMedia(config, downloadCode, log) {
    if (!config.robotCode) {
        if (log?.error) {
    ...
    try {
        ...
        const response = await axios.post('https://api.dingtalk.com/v1.0/robot/messageFiles/download', { downloadCode, robotCode: config.robotCode }, { headers: { 'x-acs-dingtalk-access-token': token } });
...
async function sendInteractiveCard(config, conversationId, text, options = {}) {
...
async function updateInteractiveCard(config, cardBizId, text, options = {}) {
    const token = await getAccessToken(config, options.log);
    // Extract title and detect markdown
    const { useMarkdown, title } = detectMarkdownAndExtractTitle(text, options, 'Clawdbot 消息');
...
async function updateInteractiveCardThrottled(config, cardBizId, text, options = {}) {
    const now = Date.now();
    ...
    if (isDirect) {
        ...
        if (dmPolicy === 'allowlist') {
            ...
            if (!isAllowed) {
                ...
                try {
                    await sendBySession(dingtalkConfig, sessionWebhook, `⛔ 访问受限\n\n您的用户ID：\`${senderId}\`\n\n请联系管理员将此ID添加到允许列表中。`, { log });
                }
                catch (err) {
...
export const dingtalkPlugin = {
    ...
    config: {
        ...
        isConfigured: (account) => Boolean(account.config?.clientId && account.config?.clientSecret),
    ...
    groups: {
        resolveRequireMention: ({ cfg }) => getConfig(cfg).groupPolicy !== 'open',
    },
    messaging: {
        normalizeTarget: ({ target }) => (target ? { targetId: target.replace(/^(dingtalk|dd|ding):/i, '') } : null),
        targetResolver: { looksLikeId: (id) => /^[\w-]+$/.test(id), hint: '<conversationId>' },
    },
    outbound: {
        deliveryMode: 'direct',
        resolveTarget: ({ to }) => {
